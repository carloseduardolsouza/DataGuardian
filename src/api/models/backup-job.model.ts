...
export async function createBackupJob(data: CreateBackupJobData) {
  ...
  if (!datasource) {
    throw new AppError('NOT_FOUND', 404, `Datasource '${data.datasource_id}' notificação encontrado`);
  }
  if (!storageLocation) {
    throw new AppError('NOT_FOUND', 404, `Storage location '${primaryStorageId}' notificação encontrado`);
  }
  ...
  if (storageTargets.length > 1) {
    ...
    if (count !== uniqueIds.length) {
      throw new AppError('NOT_FOUND', 404, 'Um ou mais storage targets notificação foram encontrados');
    }
  }
...
export async function updateBackupJob(id: string, data: UpdateBackupJobData) {
  ...
  if (data.datasource_id) {
    ...
    if (!ds) throw new AppError('NOT_FOUND', 404, `Datasource '${data.datasource_id}' notificação encontrado`);
  }
