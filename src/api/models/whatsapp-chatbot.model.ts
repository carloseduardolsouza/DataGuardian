...
async function buildMachineMessage() {
  ...
  return [
    ...
    `database: ${dbCheck}`,
    `redis: ${redis}`,
    `workers: backup=${workers.backup.status}, restore=${workers.restore.status}, scheduler=${workers.scheduler.status}, db_sync=${workers.db_sync.status}, restore_drill=${workers.restore_drill.status}, health=${workers.health.status}, cleanup=${workers.cleanup.status}`,
